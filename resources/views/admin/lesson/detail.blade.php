            ...
            <div
                ...
                <div class="flex justify-between items-center mb-4">
                    <h4 class="text-xl font-bold text-black dark:text-white">
                        Danh sách video của bài học: {{ $lesson -> LessonName }}
                    </h4>
                    <a href="{{ route('lesson.add-video',['id' =>$lesson->id]) }}"
                    ...
                    <div class="flex flex-col gap-7.5">
                        <div
                            ...
                            <div class="flex items-center justify-between px-8">
                                <div class="flex gap-5 sm:p-6 xl:p-10">
                                    <iframe class="w-2/12 aspect-video" src="{{ $video->LessonLinkUrl }}"
                                            allowfullscreen></iframe>
                                    <div class="text-xl bold">
                                        {{ $video->Title}}
                                    </div>
                                </div>
                                <form class="deleteLessonVideo"
                                      data-redirect="{{ route('lesson.detail', ['id' => $lesson->id]) }}"
                                      method="GET"
                                      data-url="{{ route('lessonVideo.destroy', $video->id) }}">
                                    @csrf
                                    <button type="submit" class="text-danger" style="margin-top: 5px;">
