<header
    class="sticky top-0 z-999 flex w-full bg-white dark:bg-boxdark dark:drop-shadow-none rounded-2xl"
>
    <div
        class="flex flex-grow items-center justify-between py-4 px-4 shadow-2 md:px-6 2xl:mx-11 rounded-2xl dark:shadow-none dark:bg-boxdark2"
    >
      ...
      <span class="relative block h-5.5 w-5.5 cursor-pointer">
        ...
        <span class="hidden text-right lg:block">
          ...
          >{{ auth() ->user() -> lastName . ' ' . auth() ->user() -> firstName }}</span
          >
            @php $roleName = DB::table('roles')->where('id', auth()->user()->roleID)->value('name') @endphp
          <span class="block text-xs font-medium dark:text-white">{{ $roleName }}</span>
        </span>

                    <span class="h-12 w-12 rounded-full">
          <img src="{{ asset(auth() -> user() -> avatar) }}" class="rounded-full" style="object-fit: cover; width: 100%; height: 100%" alt="User"/>
        </span>
