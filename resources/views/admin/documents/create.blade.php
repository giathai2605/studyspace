...
@section('content')
    <div class="mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10">
        <div class="rounded-md border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
            ...
            <form id="documentFormCreate" action="{{ route('documents.store') }}" method="post"
                data-url="{{ route('documents.store') }}" enctype="multipart/form-data">
                @csrf
                <div class="p-6.5">
                    <div class="mb-4.5">
                        ...
                        <input name="name" type="text" placeholder="Nhập tên tài liệu" value="{{ old('name') }}"
                    ...
                    <div class="">
                        ...
                        <textarea rows="6" placeholder="Nhập mô tả" name="description"
                            class="w-full rounded-lg border-[1.5px] border-stroke bg-transparent py-3 px-5 font-medium outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary">{{ old('LessonDescription') }}</textarea>
                        @error('description')
                            <span class="text-sm text-red-500">{{ $message }}</span>
                        @enderror
                    </div>
